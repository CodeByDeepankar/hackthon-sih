    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white p-4 md:p-8">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <Link href="/student" className="text-blue-600 hover:underline text-sm">← Back to Dashboard</Link>
        </div>

        <div className="mb-4 flex flex-wrap gap-2">
          {['answer','explain','practice'].map(m => (
            <button key={m} onClick={()=>setMode(m)} className={`px-3 py-1 rounded-full text-sm border ${mode===m? 'bg-blue-600 text-white border-blue-600':'bg-white hover:bg-blue-50'}`}>{m}</button>
          ))}
        </div>
        ...
        <div className="bg-white border rounded-xl h-[60vh] flex flex-col overflow-hidden shadow-sm">
          <div className="flex-1 overflow-y-auto p-4 space-y-4 text-sm">
            {history.length === 0 && (
              <div className="text-gray-500 text-center pt-10">
                Ask any STEM question. Modes:
                <ul className="list-disc list-inside text-left mt-2 space-y-1">
                  <li><strong>answer</strong>: Quick answer + short explanation</li>
                  <li><strong>explain</strong>: Deeper concept breakdown</li>
                  <li><strong>practice</strong>: Answer + follow-up practice questions</li>
                </ul>
              </div>
            )}
            {history.map((m,i)=>(
            ...
            {!loading && !history.length && !error && (
              <div className="text-[11px] text-gray-400">
                Tip: Try asking things like &quot;explain photosynthesis&quot; or &quot;practice integer addition&quot;.
              </div>
            )}
          ...
          <div className="border-t p-3 bg-slate-50">
            ...
            <div className="flex justify-between mt-2 text-xs text-gray-500">
              <span>{mode === 'practice' ? 'Will include practice questions.' : mode === 'explain' ? 'Detailed explanation mode.' : 'Concise answer mode.'}</span>
              <div className="flex gap-2">
                <button disabled={loading || !input.trim()} onClick={send} className="px-4 py-1.5 rounded bg-blue-600 text-white text-xs font-medium disabled:opacity-50">Send</button>
                <button disabled={history.length===0} onClick={()=>setHistory([])} className="px-3 py-1.5 rounded border text-xs">Clear</button>
              </div>
            </div>
          </div>
        </div>

        <p className="mt-4 text-[11px] text-gray-500">AI answers may contain mistakes. Verify critical information.</p>
      </div>
    </div>
  );
}
